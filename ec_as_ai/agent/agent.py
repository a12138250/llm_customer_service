...
from ec_as_ai.agent.message_processor import (
    MessageProcessor,
    ProcessorConfig,
    MessageResponse,
)
from ec_as_ai.agent.actions import register_action, Action
from ec_as_ai.agent.graph import (
    get_message_processing_graph,
    create_initial_state,
)
from ec_as_ai.core.tracker import DialogueStateTracker
from ec_as_ai.core.domain import Domain
from ec_as_ai.core.stores import create_tracker_store, TrackerStore
from ec_as_ai.dialogue_understanding.flow import FlowsList, FlowLoader
...
class Agent:
    ...
    def load(
        ...
        """
        import tempfile
        from ec_as_ai.training.model_storage import (
        ...
        if project_path.is_file() and project_path.name.endswith(".tar.gz"):
            ...
            # 解压到临时目录
            temp_dir = tempfile.mkdtemp(prefix="ec_as_model_")
        ...
        from ec_as_ai.shared.config import EndpointsConfig
        ...
        if config_path.exists():
            ...
            if config_data:
                ...
                if "retrieval" in config_data:
                    from ec_as_ai.shared.config import RetrievalConfig
        ...
        if llm_config:
            from ec_as_ai.dialogue_understanding.generator import (
                LLMCommandGenerator,
                LLMGeneratorConfig,
            )
        ...
        if retriever_class_path:
            try:
                from ec_as_ai.retrieval import create_retriever
        ...
        if nlg_config and nlg_config.rephrase_enabled:
            try:
                from ec_as_ai.nlg import ResponseRephraser, RephraserConfig, TemplateNLG
