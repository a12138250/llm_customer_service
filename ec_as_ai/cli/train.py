...
def train_command(
    ...
    示例:
        ec_as train
        ec_as train --data ./my_data --output ./my_models
        ec_as train --dry-run  # 仅验证
    """
    verbose = ctx.obj.get("verbose", False)
    debug = ctx.obj.get("debug", False)
    
    click.echo("=" * 50)
    click.echo("EcAs AI - 模型训练")
    click.echo("=" * 50)
    ...
    try:
        # 导入训练模块
        from ec_as_ai.training.trainer import Trainer, TrainerConfig
        from ec_as_ai.shared.config import EcAsConfig
        
        # 加载配置（用于验证）
        click.echo("加载配置...")
        ec_as_config = EcAsConfig.load(str(config_path))
        click.echo("  配置加载成功")
        ...
        if dry_run:
            ...
            from ec_as_ai.dialogue_understanding.flow import FlowLoader
