...
目录结构：
    ...
    ├── requirements-ec_as.txt     <-- 依赖列表也放在这里
    ├── ec_as_ai/                  <-- 要安装的 Python 包
    │   ├── __init__.py
    │   ├── agent/
    │   ├── core/
    │   └── ...
...
- ec_as_ai/ 是我们要安装的包（里面有 __init__.py）
- 所以 setup.py 放在 ec_as_ai/ 的父目录，即项目根目录
...
2. ec_as_ai 包会被注册到 Python 环境，可以在任何地方 import
3. ec_as 命令会被注册到系统，可以在命令行直接使用：
   - ec_as init    # 初始化项目
   - ec_as train   # 训练模型
   - ec_as run     # 启动服务
   - ec_as inspect # 交互式测试
...
Q: 安装后怎么卸载？
A: pip uninstall ec_as_ai

Q: 怎么查看是否安装成功？
A: pip show ec_as_ai  或者  python -c "import ec_as_ai; print(ec_as_ai.__version__)"
"""
...
def read_version() -> str:
    """从 ec_as_ai/__init__.py 读取版本号。
    
    为什么版本号定义在 __init__.py 而不是这里？
    - 这样代码里可以通过 ec_as_ai.__version__ 获取版本
    - 版本号只需要维护一处，不会出现不一致
    """
    init_path = Path(__file__).parent / "ec_as_ai" / "__init__.py"
...
# 核心依赖：运行 ec_as_ai 必须的包
# 从 requirements-ec_as.txt 文件读取
install_requires = read_requirements("requirements-ec_as.txt")
...
setup(
    ...
    # 
    # include: 只包含这些包（ec_as_ai 和它的所有子包）
    ...
    # 命令行入口点（CLI）
    # ------------------------------------------------------------------------
    # 安装后会创建 ec_as 命令，执行时调用 ec_as_ai.cli 模块的 main 函数
    # 
    # 格式："命令名=模块路径:函数名"
    # 安装后可以直接在终端运行：ec_as init / ec_as train / ec_as run
    entry_points={
        "console_scripts": [
            "ec_as=ec_as_ai.cli:main",
        ],
    },
